...
// react 3 starter template
const templateFiles = fs
   .readdirSync(join(CLI_PATH, 'template-react-tailwind-ts'))
   // _gitignore is renamed to .gitignore
   .map((filePath) => {
      if (filePath === '_gitignore') {
         return '.gitignore'
      }
      if (filePath === '_npmrc') {
         return '.npmrc'
...
test('successfully scaffolds a project based on react starter template', () => {
   const { stdout } = run([projectName, '--template', 'react-tailwind-ts'], {
      cwd: __dirname,
   })
...
test('works with the -t alias', () => {
   const { stdout } = run([projectName, '-t', 'react-tailwind-ts'], {
      cwd: __dirname,
   })
   const generatedFiles = fs.readdirSync(genPath).sort()
